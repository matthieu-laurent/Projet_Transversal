...
// advantage of double-buffering (checking the TXBSY flag) using polling.
//
//
// Typical Write:
//
         ...
         switch (Command)
         {
            ...
            default:
               state = 2;              // Any errors in the Command parsing
                                       // should go to state 2 where SLVSEL
                                       // is de-asserted
...
void SPI_Byte_Write (void)
{ 
   while (!SLVSEL);                    // Wait until the SPI is free, in case
                                       // it's already busy
...
// being sent using the TXBSY flag.
//
//-----------------------------------------------------------------------------
void SPI_Array_Write (void)
{
   ...
   EIE1 &= ~0x01;                      // Disable SPI interrupts
 
   SLVSEL = 0;
 
   SPI0DAT = SPI_WRITE_BUFFER;         // Load the XMIT register
   while (TXBSY != 1)                  // Wait until the command is moved into
   {
                                   // the XMIT buffer
   ...
   for (array_index = 0; array_index < MAX_BUFFER_SIZE; array_index++)
   {
      ...
      while (TXBSY != 1)               // Wait until the data is moved into
      {
                                // the XMIT buffer
   ...
   SLVSEL = 1;                         // Diable the Slave
 
   EIE1 |= 0x01;                       // Re-enable SPI interrupts
}
